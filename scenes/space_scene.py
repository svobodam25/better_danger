...
class SpaceScene:
    ...
    def _generate_nearby_planets(self):
        """Generate undiscovered planets near the player."""
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_m:
                ...
                if self.player.warp_cooldown <= 0 and self.player.fuel >= cfg.WARP_FUEL_COST:
                    # Warp: jump to a random position ~20000 px away
                    angle = random.uniform(0, 2 * math.pi)
                    dist = random.uniform(15000, 25000)
    ...
    def draw(self, screen):
        ...
        # Draw known planets
        for pid, planet in self.player.known_planets.items():
            sx = cx + planet.x
            sy = cy + planet.y
